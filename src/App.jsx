...
const SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImxhZ29vbnJrYmFzdG9meGthdG94Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzUyNjc3MTYsImV4cCI6MjA5MDg0MzcxNn0.mjvo6wqiuL87GFNWxjNoQ0jIMrZ0npuBLr1hzVK-sEo';

const sb = {
  async getUser(username) {
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/users?username=eq.${encodeURIComponent(username)}&limit=1`, {
  ...
  async getUserData(username, key) {
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/user_data?username=eq.${encodeURIComponent(username)}&key=eq.${encodeURIComponent(key)}&limit=1`, {
        headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }
      });
      const data = await r.json();
...
// localStorage fallback shim for session persistence
const ls = {
  get: (key) => new Promise((resolve) => {
    try { const val = localStorage.getItem(key); resolve(val ? { value: val } : null); } catch(e) { resolve(null); }
  }),
...
const LEARN = {
  lr: {
    title: "Logical Reasoning", color: "bg-slate-800",
  ...
  rc: {
    title: "Reading Comprehension", color: "bg-slate-700",
  ...
  lsat: {
    title: "The LSAT Structure", color: "bg-amber-700",
...
export default function SpecterLSATApp() {
  ...
  const [achievements, setAchievements] = useState({ firstQuestion: false, firstTest: false, firstArticle: false, q100: false, q1000: false, allQuestions: false, timedLSAT: false });
  ...
  useEffect(() => {
    if (user && user.username && Object.keys(completed).length > 0) {
      ls.set("ct_" + user.username, JSON.stringify(completed)).catch(() => {});
      sb.setUserData(user.username, 'completed', JSON.stringify(completed)).catch(() => {});
    }
  }, [completed, user]);
  useEffect(() => {
    if (user && user.username && Object.keys(testHistory).length > 0) {
      ls.set("th_" + user.username, JSON.stringify(testHistory)).catch(() => {});
      sb.setUserData(user.username, 'testHistory', JSON.stringify(testHistory)).catch(() => {});
    }
  }, [testHistory, user]);
  useEffect(() => {
  ...
  const doLogin = async () => {
    ...
    // Admin accounts
    const admins = [
      { u: "admin1", p: "Leroybrown", email: "admin@specterlsat.com" },
      { u: "influencer1", p: "influencerpass1", email: "influencer@specterlsat.com" },
      { u: "Admin", p: "Leroybrown22!", email: "admin@specterlsat.com" },
    ];
    const admin = admins.find(a => a.u === username && a.p === password);
    if (admin) {
    ...
    try {
      // Try Supabase first
      const sbUser = await sb.getUser(username);
      if (sbUser) {
        if (sbUser.password !== password) return alert("Incorrect password");
        const u = { username: sbUser.username, email: sbUser.email, isPremium: sbUser.is_premium || false, hasTestAccess: sbUser.has_test_access || false, createdAt: sbUser.created_at };
        try { await ls.set("su", JSON.stringify(u)); } catch(e) {}
  ...
  const doSignUp = async () => {
    ...
    try {
      ...
      if (existing) return alert("Username already taken. Please choose another.");
      const userData = { username, email, password, is_premium: false, has_test_access: false, created_at: new Date().toISOString() };
      await sb.upsertUser(userData);
      const u = { username, email, password, isPremium: false, hasTestAccess: false, createdAt: new Date().toISOString() };
      try { await ls.set("u_" + username, JSON.stringify(u)); } catch(e) {}
      setForm({ username: "", email: "", password: "" }); setIsLogin(true);
  ...
  const upgrade = async (premium) => {
    const url = premium
      ? 'https://buy.stripe.com/4gMbIU5IseEsdKL16T7EQ07'
      : 'https://buy.stripe.com/9B65kw0o8dAo2233f17EQ06';
    window.open(url, '_blank');
  };
  
  const activateSubscription = async (premium) => {
    if (!user) return;
    const updated = { ...user, hasTestAccess: true, ...(premium ? { isPremium: true } : {}) };
    try { await ls.set("su", JSON.stringify(updated)); } catch(e) {}
    // Update Supabase
    await sb.upsertUser({ username: user.username, email: user.email, password: user.password || '', is_premium: updated.isPremium || false, has_test_access: true, created_at: user.createdAt });
    setUser(updated);
  };
  ...
  const doAdvance = () => {
    ...
    setTScores(ns);
    if (tPartIdx >= tParts.length - 1) { setTDone(true); setTActive(false); setAchievements(a => ({...a, timedLSAT: true})); }
  ...
  if (showHome && !user) return (
    <div className="min-h-screen bg-slate-950">
      <nav className="bg-slate-900 shadow-md border-b border-slate-700"><div className="max-w-7xl mx-auto px-4 md:px-6 flex justify-between items-center h-16 md:h-20"><div className="flex items-center gap-2 md:gap-3"><Suit /><span className="text-base md:text-xl font-bold text-white">Specter LSAT Prep</span></div><div className="flex items-center gap-1 md:gap-3"><button onClick={() => setShowPricing(true)} className="text-white font-semibold hover:text-amber-400 px-2 md:px-4 py-2 text-sm md:text-base">Pricing</button><button onClick={() => setShowContact(true)} className="text-white font-semibold hover:text-amber-400 px-2 md:px-4 py-2 text-sm md:text-base">Contact</button><button onClick={() => { const el = document.getElementById("app-download-section"); if (el) el.scrollIntoView({behavior:"smooth"}); }} className="text-white font-semibold hover:text-amber-400 px-2 md:px-4 py-2 text-sm md:text-base">App Download</button><button onClick={() => setShowHome(false)} className="bg-amber-400 text-slate-900 px-4 md:px-8 py-2 md:py-3 rounded-lg text-sm md:text-base font-semibold hover:bg-amber-300">Get Started</button></div></div></nav>
      <div className="max-w-7xl mx-auto px-6 py-20">
        <div className="text-center mb-16 py-16 px-8 bg-slate-900 rounded-3xl shadow-2xl">
          <p className="text-amber-400 text-sm font-bold uppercase tracking-widest mb-4">The Smarter Way to Prep</p>
          <h1 className="text-6xl font-bold text-white mb-6">Master the LSAT with Confidence</h1>
          <p className="text-xl text-slate-300 mb-10 max-w-3xl mx-auto">Comprehensive practice materials designed to get you into the law school you deserve.</p>
          <button onClick={() => setShowHome(false)} className="bg-amber-400 text-slate-900 px-12 py-4 rounded-lg font-bold text-xl hover:bg-amber-300 shadow-xl">Start Practicing Now</button>
          <p className="text-slate-500 text-xs mt-4">2026 Specter LSAT Prep. Not affiliated with LSAC.</p>
        </div>
        <div className="grid md:grid-cols-3 gap-8 mb-16">
          {[{n:"2600",l:"Practice Questions",d:"Comprehensive LR and RC coverage"},{n:"100",l:"Full Practice Tests",d:"Timed practice exams per section"},{n:"100%",l:"Detailed Explanations",d:"Learn from every mistake"}].map((s,i) => (
            <div key={i} className="bg-slate-900 rounded-xl shadow-lg p-8 text-center border border-slate-700"><div className="text-5xl font-bold text-amber-400 mb-2">{s.n}</div><div className="text-xl font-semibold text-white mb-2">{s.l}</div><p className="text-slate-400">{s.d}</p></div>
          ))}
        </div>
        <div className="bg-slate-800 rounded-2xl shadow-xl p-12 mb-16 border border-slate-200">
          <h2 className="text-4xl font-bold text-white text-center mb-12">Why Choose Specter LSAT Prep?</h2>
          <div className="grid md:grid-cols-2 gap-8">
            {[{I:Brain,t:"Expert-Crafted Questions",d:"Every question mirrors the style and difficulty of actual LSAT questions."},{I:BookOpen,t:"Comprehensive Coverage",d:"1,300 questions per section in manageable 26-question practice tests."},{I:CheckCircle,t:"Instant Feedback",d:"Get immediate results after each question to track your progress."},{I:Crown,t:"Premium Explanations",d:"Understand why correct answers are right and wrong answers are wrong."},{I:BookOpen,t:"Free Learn Section",d:"25 in-depth strategy articles covering every question type. Free for all users.",b:"Free"},{I:List,t:"Timed Full LSAT",d:"Full timed exam with two LR sections and one RC section, 35 min each.",b:"Premium"},{I:CheckCircle,t:"Drill Mode",d:"Unlimited random questions from the full bank. Track your accuracy in real time. Free for all users -- premium users unlock detailed explanations after every question.",b:"Free"}].map(({I,t,d,b},i) => (
              <div key={i} className="flex gap-4"><div className="w-12 h-12 bg-slate-900 rounded-lg flex items-center justify-center flex-shrink-0 border border-amber-400"><I className="w-6 h-6 text-amber-400" /></div><div><h3 className="text-lg font-bold text-white mb-1">{t}{b && <span className={`text-xs font-semibold ml-2 px-2 py-0.5 rounded-full ${b==="Free"?"bg-amber-400 text-slate-900":"bg-slate-700 text-amber-400 border border-amber-400"}`}>{b}</span>}</h3><p className="text-slate-300 text-sm">{d}</p></div></div>
            ))}
          </div>
        </div>
        <div id="app-download-section" className="bg-slate-800 rounded-2xl shadow-2xl p-12 mb-16">
          <h2 className="text-4xl font-bold text-white text-center mb-4">Download the App</h2>
          <p className="text-slate-300 text-center mb-8">Add Specter to your home screen for instant access — no App Store required.</p>
          <div className="flex gap-3 justify-center mb-8">
            <button onClick={()=>setInstallPlatform('ios')} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold border-2 transition-all ${installPlatform==='ios'?'border-amber-500 text-amber-600 bg-slate-700':'border-slate-600 text-slate-400 hover:border-gray-400'}`}>
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M18.71 19.5c-.83 1.24-1.71 2.45-3.05 2.47-1.34.03-1.77-.79-3.29-.79-1.53 0-2 .77-3.27.82-1.31.05-2.3-1.32-3.14-2.53C4.25 17 2.94 12.45 4.7 9.39c.87-1.52 2.43-2.48 4.12-2.51 1.28-.02 2.5.87 3.29.87.78 0 2.26-1.07 3.8-.91.65.03 2.47.26 3.64 1.98-.09.06-2.17 1.28-2.15 3.81.03 3.02 2.65 4.03 2.68 4.04-.03.07-.42 1.44-1.38 2.83M13 3.5c.73-.83 1.94-1.46 2.94-1.5.13 1.17-.34 2.35-1.04 3.19-.69.85-1.83 1.51-2.95 1.42-.15-1.15.41-2.35 1.05-3.11z"/></svg>
              iPhone / iPad
            </button>
            <button onClick={()=>setInstallPlatform('android')} className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold border-2 transition-all ${installPlatform==='android'?'border-amber-500 text-amber-600 bg-slate-700':'border-slate-600 text-slate-400 hover:border-gray-400'}`}>
              <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor"><path d="M17.523 15.341c-.494 0-.9-.405-.9-.9s.406-.9.9-.9.9.405.9.9-.406.9-.9.9m-11.046 0c-.494 0-.9-.405-.9-.9s.406-.9.9-.9.9.405.9.9-.406.9-.9.9m11.405-6.009l1.8-3.118a.375.375 0 0 0-.136-.511.375.375 0 0 0-.512.136l-1.823 3.155A11.09 11.09 0 0 0 12 8.1c-1.65 0-3.21.386-4.61 1.069L5.565 5.839a.375.375 0 0 0-.648.375l1.8 3.118C4.108 10.773 2.25 13.5 2.25 16.65h19.5c0-3.15-1.858-5.877-3.868-7.318"/></svg>
              Android
            </button>
          </div>
          <div className="bg-slate-700 rounded-2xl p-6 border border-slate-600">
            <p className="text-xs font-bold text-amber-600 uppercase tracking-widest mb-5">
              {installPlatform === 'ios' ? 'HOW TO INSTALL ON IPHONE / IPAD' : 'HOW TO INSTALL ON ANDROID'}
            </p>
            {installPlatform === 'ios' ? (
              <div className="space-y-3">
                {[
                  { n:1, text: <span>Tap the <strong>Share button</strong> at the bottom of Safari — the box with an arrow pointing up</span>, icon: '↑' },
                  { n:2, text: <span>Scroll down the menu and tap <strong>"Add to Home Screen"</strong></span>, icon: '+' },
                  { n:3, text: <span>Tap <strong>"Add"</strong> in the top right corner — Specter appears on your home screen instantly</span>, icon: '✓' }
                ].map(({n,text,icon})=>(
                  <div key={n} className="flex items-center gap-4 bg-slate-700 rounded-xl p-4 border border-slate-600 shadow-sm">
                    <div className="w-9 h-9 rounded-full bg-slate-800 text-white font-bold text-sm flex items-center justify-center flex-shrink-0">{n}</div>
                    <p className="text-slate-200 text-sm flex-1">{text}</p>
                    <span className="text-amber-400 text-lg flex-shrink-0 font-bold">{icon}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="space-y-3">
                {[
                  { n:1, text: <span>Open your browser menu — tap the <strong>three dots</strong> in the top right corner</span>, icon: '⋮' },
                  { n:2, text: <span>Tap <strong>"Add to Home Screen"</strong> or <strong>"Install App"</strong> from the menu</span>, icon: '+' },
                  { n:3, text: <span>Tap <strong>"Add"</strong> to confirm — Specter appears on your home screen instantly</span>, icon: '✓' }
                ].map(({n,text,icon})=>(
                  <div key={n} className="flex items-center gap-4 bg-slate-700 rounded-xl p-4 border border-slate-600 shadow-sm">
                    <div className="w-9 h-9 rounded-full bg-slate-800 text-white font-bold text-sm flex items-center justify-center flex-shrink-0">{n}</div>
                    <p className="text-slate-200 text-sm flex-1">{text}</p>
        ...
        <div className="mb-16">
          <h2 className="text-4xl font-bold text-white text-center mb-10">Affordable, Effective Preparation</h2>
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            <div className="bg-slate-800 rounded-2xl shadow-xl p-8 border-2 border-slate-600"><h3 className="text-2xl font-bold text-white mb-3">Test Access</h3><div className="mb-4"><span className="text-5xl font-bold text-white">$20</span><span className="text-xl text-slate-400">/month</span></div>{["All 2,600 questions","100 practice tests","Instant score feedback"].map((x,i)=><div key={i} className="flex items-center gap-2 text-slate-200 mb-2"><CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0"/>{x}</div>)}</div>
            <div className="bg-gradient-to-br from-slate-800 to-slate-700 rounded-2xl shadow-xl p-8 text-white relative"><div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-amber-400 text-white text-sm px-4 py-1 rounded-full font-bold">BEST VALUE</div><h3 className="text-2xl font-bold mb-3">Premium Access</h3><div className="mb-4"><span className="text-5xl font-bold">$25</span><span className="text-xl opacity-90">/month</span></div>{["Everything in Test Access","Detailed answer explanations","Learn from every mistake"].map((x,i)=><div key={i} className="flex items-center gap-2 mb-2"><CheckCircle className="w-5 h-5 text-amber-300 flex-shrink-0"/>{x}</div>)}</div>
          </div>
        </div>
      </div>
      <footer className="bg-slate-900 text-white py-12 border-t border-slate-700"><div className="max-w-7xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4"><div className="flex items-center gap-3"><Suit size={8}/><span className="text-2xl font-bold">Specter LSAT Prep</span></div><div className="flex items-center gap-6"><button onClick={() => setShowContact(true)} className="text-slate-500 hover:text-white">Contact Us</button><button onClick={() => setShowPrivacy(true)} className="text-slate-400 hover:text-amber-400 transition-colors">Privacy Policy</button><button onClick={() => setShowTerms(true)} className="text-slate-400 hover:text-amber-400 transition-colors">Terms of Service</button></div></div><p className="text-center text-slate-500 text-sm mt-6">2026 Specter LSAT Prep. Not affiliated with LSAC.</p></footer>
      {showPricing && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-slate-800 rounded-2xl max-w-3xl w-full p-6 md:p-10 shadow-2xl my-4 md:my-8">
            <div className="flex justify-between items-center mb-6 md:mb-8">
              <div>
                <h2 className="text-2xl md:text-3xl font-bold text-white">Simple, Transparent Pricing</h2>
                <p className="text-slate-400 mt-1 text-sm md:text-base">Choose the plan that fits your preparation goals</p>
              </div>
              <button onClick={() => setShowPricing(false)} className="text-slate-500 hover:text-slate-300 text-3xl font-bold leading-none ml-4">x</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8 mb-6 md:mb-8">
              <div className="border-2 border-slate-600 rounded-2xl p-6 md:p-8 hover:border-amber-400 transition-all">
                <h3 className="text-2xl font-bold text-white mb-2">Test Access</h3>
                <div className="mb-4 md:mb-6"><span className="text-5xl md:text-6xl font-bold text-white">$20</span><span className="text-xl text-slate-400">/month</span></div>
                <p className="text-slate-300 mb-4 md:mb-6 text-sm md:text-base">Everything you need to practice at full volume with real exam conditions.</p>
                <ul className="space-y-3 md:space-y-4 mb-6 md:mb-8">
                  {[["2,600 total practice questions","1,300 per section across LR and RC"],["100 full practice tests","50 per section, 26 questions each"],["Randomized answer positions","No predictable patterns"],["Timed Full Test mode","Simulate real LSAT with auto-advancing sections"],["Instant score feedback","See your results after every question"],["Free Learn section","25 in-depth strategy articles included"],["Drill Mode access","Unlimited random questions -- track your accuracy live"]].map(([title, desc], i) => (
                    <li key={i} className="flex gap-3">
                      ...
                      <div><p className="font-semibold text-slate-100 text-sm md:text-base">{title}</p><p className="text-xs md:text-sm text-slate-400">{desc}</p></div>
                    </li>
                  ))}
                  <li className="flex gap-3"><XCircle className="w-5 h-5 text-gray-300 flex-shrink-0 mt-0.5" /><div><p className="font-semibold text-slate-500 text-sm md:text-base">No answer explanations</p><p className="text-xs md:text-sm text-slate-500">Upgrade to Premium to unlock</p></div></li>
                </ul>
                <a href="https://buy.stripe.com/9B65kw0o8dAo2233f17EQ06" target="_blank" rel="noopener noreferrer" className="w-full bg-gray-900 text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg hover:bg-gray-800 transition-all block text-center">Get Test Access</a>
              </div>
              <div className="border-2 border-amber-500 rounded-2xl p-6 md:p-8 bg-gradient-to-br from-amber-50 to-slate-50 relative mt-4 md:mt-0">
                <div className="absolute -top-4 left-1/2 -translate-x-1/2 bg-gradient-to-r from-slate-800 to-slate-700 text-white px-6 py-1.5 rounded-full text-sm font-bold">BEST VALUE</div>
                <h3 className="text-2xl font-bold text-white mb-2">Premium Access</h3>
                <div className="mb-4 md:mb-6"><span className="text-5xl md:text-6xl font-bold text-white">$25</span><span className="text-xl text-slate-400">/month</span></div>
                <p className="text-slate-300 mb-4 md:mb-6 text-sm md:text-base">Everything in Test Access, plus the explanations that accelerate your learning.</p>
                <ul className="space-y-3 md:space-y-4 mb-6 md:mb-8">
                ...
                <a href="https://buy.stripe.com/4gMbIU5IseEsdKL16T7EQ07" target="_blank" rel="noopener noreferrer" className="w-full bg-gradient-to-r from-slate-800 to-slate-700 text-white py-3 md:py-4 rounded-xl font-bold text-base md:text-lg hover:from-slate-900 hover:to-slate-800 transition-all block text-center">Get Premium Access</a>
              </div>
            </div>
            <div className="bg-slate-700 rounded-xl p-4 md:p-6 text-center mb-4">
              <p className="text-slate-300 text-sm"><span className="font-semibold">Not sure which plan to choose?</span> Start with Test Access and upgrade to Premium any time. Both plans include the free Learn section with 25 strategy articles.</p>
              <p className="text-slate-500 text-xs mt-2">Refunds may be provided under certain conditions. Cancel anytime to stop future charges. Governed by the laws of Florida, USA.</p>
            </div>
            <button onClick={() => { setShowPricing(false); }} className="w-full bg-gray-100 text-slate-200 py-3 rounded-xl font-semibold hover:bg-gray-200 transition-all">Back to Home</button>
          </div>
        </div>
      )}
      {showContact && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl max-w-md w-full p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <h2 className="text-2xl font-bold text-slate-100">Contact Us</h2>
              <button onClick={() => setShowContact(false)} className="text-slate-500 hover:text-slate-300 text-2xl font-bold leading-none">x</button>
            </div>
            <div className="space-y-6">
              <div className="bg-slate-700 rounded-xl p-6 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-1">Phone</p>
                  <a href="tel:9044828164" className="text-lg font-bold text-slate-100 hover:text-amber-600">(904) 482-8164</a>
                </div>
              </div>
              <div className="bg-slate-700 rounded-xl p-6 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-1">TikTok</p>
                  <a href="https://www.tiktok.com/@specterlsatprep" target="_blank" rel="noopener noreferrer" className="text-lg font-bold text-slate-100 hover:text-pink-600">@specterlsatprep</a>
                </div>
              </div>
              <div className="bg-slate-700 rounded-xl p-6 flex items-center gap-4">
                <div className="w-12 h-12 bg-slate-700 rounded-full flex items-center justify-center flex-shrink-0">
                ...
                <div>
                  ...
                  <a href="mailto:specterlsatprep@gmail.com" className="text-lg font-bold text-slate-100 hover:text-amber-600">specterlsatprep@gmail.com</a>
                </div>
              </div>
            </div>
            <button onClick={() => setShowContact(false)} className="w-full mt-6 bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900">Close</button>
          </div>
        </div>
      )}
      {showPrivacy && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-slate-800 rounded-2xl max-w-lg w-full p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              ...
              <button onClick={()=>setShowPrivacy(false)} className="text-slate-500 hover:text-slate-300 text-2xl font-bold leading-none">x</button>
            </div>
            <div className="text-sm text-slate-200 space-y-4 max-h-96 overflow-y-auto pr-2">
              <p className="text-xs text-slate-500">Last updated: April 2026</p>
              <div><p className="font-bold text-slate-100 mb-1">1. Information We Collect</p><p>We collect your username, email address, and password when you create an account. We also collect your practice test scores, drill history, and progress data to provide our service.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">2. How We Use Your Information</p><p>We use your information to operate and improve Specter LSAT Prep, process your subscription payments through Stripe, and communicate with you about your account. We do not sell your personal information to third parties.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">3. Data Storage</p><p>Your account information is stored securely in Supabase, a cloud database service. Payment processing is handled entirely by Stripe. We do not store your credit card information.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">4. Passwords</p><p>Your password is stored in our database. We strongly recommend using a unique password for your Specter account. We will never ask for your password via email.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">5. Cookies and Local Storage</p><p>We use your browser's local storage to keep you logged in between sessions. We do not use third-party tracking cookies.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">6. Third-Party Services</p><p>We use Stripe for payment processing, Supabase for data storage, and Vercel for hosting. Each has their own privacy policy governing their use of data.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">7. Your Rights</p><p>You may request deletion of your account and associated data at any time by emailing specterlsatprep@gmail.com. We will process deletion requests within 30 days.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">8. Children's Privacy</p><p>Specter LSAT Prep is not directed at children under 13. We do not knowingly collect personal information from children under 13.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">9. Changes to This Policy</p><p>We may update this Privacy Policy from time to time. Continued use of the service after changes constitutes acceptance of the updated policy.</p></div>
              <div><p className="font-bold text-slate-100 mb-1">10. Contact</p><p>For privacy-related questions, contact us at specterlsatprep@gmail.com.</p></div>
            </div>
            <button onClick={()=>setShowPrivacy(false)} className="w-full mt-6 bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900">Close</button>
          </div>
        </div>
      )}
      {showTerms && <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"><div className="bg-slate-800 rounded-2xl max-w-md w-full p-8 shadow-2xl"><h2 className="text-2xl font-bold mb-4">Terms of Service</h2><div className="text-sm text-slate-200 space-y-3 max-h-64 overflow-y-auto"><p><strong>1. Agreement.</strong> By using Specter LSAT Prep, you agree to these Terms of Service.</p><p><strong>2. Subscriptions.</strong> Test Access ($20/mo) provides access to all practice questions and tests. Premium ($25/mo) adds detailed answer explanations. Subscriptions renew monthly until cancelled.</p><p><strong>3. Refunds.</strong> Refunds may be provided under certain conditions. Contact us at specterlsatprep@gmail.com within 7 days of purchase to request a refund.</p><p><strong>4. Cancellations.</strong> You may cancel your subscription at any time through the Stripe customer portal. Access continues until the end of your billing period.</p><p><strong>5. Account Responsibility.</strong> You are responsible for maintaining the confidentiality of your account credentials. You may not share your account with others.</p><p><strong>6. Intellectual Property.</strong> All content on Specter LSAT Prep is proprietary. You may not reproduce or distribute it without permission.</p><p><strong>7. Disclaimer.</strong> Specter LSAT Prep is an independent test preparation service. We are not affiliated with or endorsed by LSAC.</p><p><strong>8. Governing Law.</strong> These terms are governed by the laws of Florida, USA.</p><p><strong>9. Contact.</strong> specterlsatprep@gmail.com</p></div><button onClick={()=>setShowTerms(false)} className="w-full mt-6 bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900">Close</button></div></div>}
    </div>
  );
  ...
  if (!user) return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-4">
      <div className="bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-8 border border-slate-700">
        <button onClick={() => setShowHome(true)} className="text-amber-400 hover:text-amber-300 mb-4 text-sm font-semibold">← Back to Home</button>
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Specter LSAT Prep</h1>
          <p className="text-slate-300">Take the first step towards mastering your LSAT</p>
        </div>
        <div className="flex gap-2 mb-6">
          {["Login","Sign Up"].map((l,i)=>(
            <button key={i} onClick={()=>setIsLogin(i===0)} className={`flex-1 py-2.5 rounded-lg font-semibold transition-all ${(i===0)===isLogin?"bg-amber-400 text-slate-900":"bg-slate-700 text-slate-300 hover:bg-slate-600 hover:text-white"}`}>{l}</button>
          ))}
        </div>
        <div className="space-y-5">
          <div>
            <label className="block text-sm font-semibold text-white mb-2">Username</label>
            <input type="text" value={form.username} onChange={e=>setForm({...form,username:e.target.value})} className="w-full px-4 py-3 bg-slate-700 border border-slate-500 text-white rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-amber-400 focus:outline-none placeholder-slate-400" placeholder="Enter your username"/>
          </div>
          {!isLogin && (
            <div>
              <label className="block text-sm font-semibold text-white mb-2">Email</label>
              <input type="email" value={form.email} onChange={e=>setForm({...form,email:e.target.value})} className="w-full px-4 py-3 bg-slate-700 border border-slate-500 text-white rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-amber-400 focus:outline-none placeholder-slate-400" placeholder="Enter your email"/>
            </div>
          )}
          <div>
            <label className="block text-sm font-semibold text-white mb-2">Password</label>
            <input type="password" value={form.password} onChange={e=>setForm({...form,password:e.target.value})} className="w-full px-4 py-3 bg-slate-700 border border-slate-500 text-white rounded-lg focus:ring-2 focus:ring-amber-400 focus:border-amber-400 focus:outline-none placeholder-slate-400" placeholder="Enter your password"/>
          </div>
          <button onClick={isLogin?doLogin:doSignUp} className="w-full bg-amber-400 text-slate-900 py-3 rounded-lg font-bold hover:bg-amber-300 flex items-center justify-center gap-2 text-base mt-2">
            {isLogin?<><LogIn className="w-5 h-5"/>Login</>:<><UserPlus className="w-5 h-5"/>Create Account</>}
          </button>
        </div>
        <div className="mt-5 text-center text-sm text-slate-300">
          {isLogin?"Don't have an account?":"Already have an account?"}
          <button onClick={()=>setIsLogin(!isLogin)} className="text-amber-400 font-semibold ml-1 hover:text-amber-300">{isLogin?"Sign up free":"Log in"}</button>
        </div>
        <div className="mt-4 text-center flex items-center justify-center gap-4 border-t border-slate-700 pt-4">
          <button onClick={()=>setShowTerms(true)} className="text-xs text-slate-400 hover:text-slate-300 underline">Terms of Service</button>
          <button onClick={()=>setShowPrivacy(true)} className="text-xs text-slate-400 hover:text-slate-300 underline">Privacy Policy</button>
        </div>
      </div>
    </div>
  );
  ...
  if (showLearn) {
    if (learnArt && learnSec) {
      ...
      if (learnArt && !articlesRead.includes(learnArt)) {
        setArticlesRead(prev => [...prev, learnArt]);
        setAchievements(a => ({...a, firstArticle: true}));
      }
      return (
        <div className="min-h-screen bg-slate-950 p-4 md:p-8">
          <div className="max-w-3xl mx-auto">
            <button onClick={()=>setLearnArt(null)} className="text-amber-600 hover:text-amber-800 mb-6 font-medium">Back to {sec.title}</button>
            ...
            <div className="bg-slate-800 rounded-2xl shadow-lg p-8 space-y-6">
              {art.sections.map((s,i)=>(
                <div key={i}><h2 className="text-sm font-bold text-slate-100 uppercase tracking-wide mb-3 pb-2 border-b-2 border-amber-200">{s.heading}</h2>{s.body&&<p className="text-slate-200 leading-relaxed">{s.body}</p>}{s.bullets&&<ul className="space-y-2 mt-2">{s.bullets.map((b,j)=><li key={j} className="flex gap-3 text-slate-200"><span className="text-amber-500 font-bold flex-shrink-0">-</span><span>{b}</span></li>)}</ul>}</div>
              ))}
            </div>
            <div className="mt-6 flex gap-3">{idx>0&&<button onClick={()=>setLearnArt(arts[idx-1].id)} className="flex-1 bg-slate-700 text-amber-400 border-2 border-amber-400 py-3 rounded-xl font-semibold hover:bg-slate-600">Previous Article</button>}{idx<arts.length-1&&<button onClick={()=>setLearnArt(arts[idx+1].id)} className="flex-1 bg-slate-800 text-white py-3 rounded-xl font-semibold hover:bg-slate-900">Next Article</button>}</div>
          </div>
        </div>
      );
    }
    if (learnSec) {
      ...
      return (
        <div className="min-h-screen bg-slate-950 p-4 md:p-8">
          <div className="max-w-3xl mx-auto">
            <button onClick={()=>setLearnSec(null)} className="text-amber-600 hover:text-amber-800 mb-6 font-medium">Back to Learn</button>
            ...
            <h2 className="text-xl font-bold text-slate-100 mb-4">Articles</h2>
            <div className="space-y-3">{sec.articles.map(art=>(
              <button key={art.id} onClick={()=>setLearnArt(art.id)} className="w-full bg-slate-800 rounded-xl shadow p-5 hover:shadow-lg hover:bg-slate-700 border-2 border-transparent hover:border-amber-300 flex items-center gap-4 text-left">
                <div className="w-12 h-12 bg-slate-900 rounded-lg flex items-center justify-center flex-shrink-0"><span className="text-xs font-bold text-amber-600">{art.icon}</span></div>
                <div className="flex-1"><h3 className="font-bold text-slate-100 mb-1">{art.title}</h3><p className="text-sm text-slate-400">{art.summary}</p></div>
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-4 md:p-8">
        <div className="max-w-3xl mx-auto">
          <button onClick={()=>setShowLearn(false)} className="text-amber-600 hover:text-amber-800 mb-6 font-medium">Back to Dashboard</button>
          <div className="text-center mb-10"><h1 className="text-4xl font-bold text-slate-100 mb-3">Learn</h1><p className="text-slate-300 text-lg">Master every question type with in-depth strategy guides</p></div>
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <button onClick={()=>setLearnSec("lr")} className="bg-slate-800 rounded-2xl p-8 text-white text-left hover:bg-slate-900"><div className="w-12 h-12 bg-white bg-opacity-20 rounded-xl flex items-center justify-center mb-4"><Brain className="w-6 h-6 text-white"/></div><h2 className="text-2xl font-bold mb-2">Logical Reasoning</h2><p className="text-amber-200 text-sm mb-3">Master all 9 LR question types with attack strategies, examples, and key tips</p><div className="text-amber-300 text-sm font-semibold">{LEARN.lr.articles.length} articles</div></button>
            <button onClick={()=>setLearnSec("rc")} className="bg-slate-700 rounded-2xl p-8 text-white text-left hover:bg-slate-800"><div className="w-12 h-12 bg-white bg-opacity-20 rounded-xl flex items-center justify-center mb-4"><BookOpen className="w-6 h-6 text-white"/></div><h2 className="text-2xl font-bold mb-2">Reading Comprehension</h2><p className="text-amber-200 text-sm mb-3">Master all 7 RC question types with active reading strategies and examples</p><div className="text-amber-300 text-sm font-semibold">{LEARN.rc.articles.length} articles</div></button>
          </div>
          <button onClick={()=>setLearnSec("lsat")} className="w-full bg-amber-700 rounded-2xl p-8 text-white text-left hover:bg-amber-800 mb-6"><div className="w-12 h-12 bg-white bg-opacity-20 rounded-xl flex items-center justify-center mb-4"><List className="w-6 h-6 text-white"/></div><h2 className="text-2xl font-bold mb-2">The LSAT Structure</h2><p className="text-amber-200 text-sm mb-3">How the test is built, timing strategies, and test day prep</p><div className="text-amber-300 text-sm font-semibold">{LEARN.lsat.articles.length} articles</div></button>
          ...
          <div className="bg-slate-800 rounded-2xl shadow p-6"><h3 className="font-bold text-slate-100 mb-2">How to use the Learn section</h3><p className="text-slate-300 text-sm">Start with the section overview, then work through the articles. After reading, head to practice tests to apply what you have learned.</p></div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-8 flex items-center justify-center">
        <div className="bg-slate-800 rounded-2xl shadow-2xl p-10 max-w-lg w-full text-center">
          <h2 className="text-3xl font-bold text-slate-100 mb-2">Full Test Complete!</h2>
          <p className="text-slate-400 mb-8">Here is how you did across all three sections</p>
          <div className="space-y-4 mb-8">
            {tScores.map((s,i)=><div key={i} className="bg-slate-700 rounded-xl p-4 flex justify-between items-center"><div className="text-left"><p className="font-semibold text-slate-100">{s.part}</p><p className="text-sm text-slate-400">{s.total} questions</p></div><div className="text-right"><p className="text-2xl font-bold text-amber-600">{s.correct}/{s.total}</p><p className="text-sm text-slate-400">{Math.round(s.correct/s.total*100)}%</p></div></div>)}
            <div className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-xl p-4 text-white flex justify-between items-center"><div className="font-bold text-lg">Total Score</div><div><p className="text-2xl font-bold">{tot}/{totQ}</p><p className="text-sm opacity-80">{Math.round(tot/totQ*100)}%</p></div></div>
          </div>
          <div className="flex gap-3"><button onClick={()=>{setTDone(false);startTimed();}} className="flex-1 bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900">Take Another</button><button onClick={()=>setTDone(false)} className="flex-1 bg-gray-200 text-slate-100 py-3 rounded-lg font-semibold hover:bg-gray-300">Back to Menu</button></div>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-4 bg-slate-800 rounded-xl shadow px-6 py-3 border border-slate-700">
            <div><span className="font-bold text-slate-100 text-lg">{part.label}</span><span className="ml-3 text-sm text-slate-400">Section {tPartIdx+1} of {tParts.length}</span></div>
            <div className={`px-4 py-2 rounded-lg font-bold text-xl ${isC?"bg-red-600 text-white":isU?"bg-orange-100 text-orange-700":"bg-slate-700 text-amber-700"}`}>{fmt(tTime)}</div>
            <button onClick={doAdvance} className="text-sm text-slate-400 hover:text-red-600 font-semibold">End Section</button>
          </div>
          <div className="flex gap-2 mb-4">{tParts.map((_,i)=><div key={i} className={`h-2 rounded-full flex-1 ${i<tPartIdx?"bg-green-500":i===tPartIdx?"bg-slate-800":"bg-gray-200"}`}/>)}</div>
          <div className="bg-slate-800 rounded-xl shadow-lg p-6 md:p-8">
            <div className="flex justify-between mb-4"><span className="text-slate-300">Question {tQ+1} of {qs.length}</span><span className="text-sm text-slate-400">{Object.keys(tAnswers).length} answered</span></div>
            <div className="w-full bg-slate-600 rounded-full h-2 mb-6"><div className="bg-slate-800 h-2 rounded-full" style={{width:`${(tQ+1)/qs.length*100}%`}}/></div>
            <div className="bg-slate-700 rounded-lg p-5 mb-5"><p className="text-slate-100 leading-relaxed">{q.passage}</p></div>
            <h3 className="text-lg font-semibold text-slate-100 mb-4">{q.question}</h3>
            <div className="space-y-3 mb-6">
              {q.options.map((opt,idx)=>{
                const s=tSel===idx; const c=idx===q.correct; let cls="bg-slate-700 hover:bg-slate-600 border-slate-600"; let ico=null;
                if(tShowRes){if(c){cls="bg-green-900 border-green-500";ico=<CheckCircle className="w-5 h-5 text-green-600"/>;}else if(s){cls="bg-red-900 border-red-500";ico=<XCircle className="w-5 h-5 text-red-600"/>;}}else if(s){cls="bg-slate-700 border-amber-500";}
                return <button key={idx} onClick={()=>{if(!tShowRes){setTSel(idx);setTAnswers(a=>({...a,[tQ]:idx}));}}} disabled={tShowRes} className={`w-full text-left p-4 rounded-lg border-2 flex items-center justify-between ${cls}`}><span className="text-slate-100">{opt}</span>{ico}</button>;
              })}
            </div>
            <div className="flex justify-between">
              <button onClick={()=>{if(tQ>0){setTQ(q=>q-1);setTSel(tAnswers[tQ-1]??null);setTShowRes(false);}}} disabled={tQ===0} className="px-6 py-2 bg-slate-700 rounded-lg hover:bg-slate-600 disabled:opacity-50">Previous</button>
              <div className="flex gap-3">
                {!tShowRes&&tSel!==null&&<button onClick={()=>setTShowRes(true)} className="px-6 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900">Submit Answer</button>}
                {tShowRes&&tQ<qs.length-1&&<button onClick={()=>{setTQ(q=>q+1);setTSel(tAnswers[tQ+1]??null);setTShowRes(false);}} className="px-6 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 flex items-center">Next<ChevronRight className="w-4 h-4 ml-1"/></button>}
  ...
  if (showExplanations) {
    ...
    if (!isPremium) {
      return (
        <div className="min-h-screen bg-slate-950 p-8 flex items-center justify-center">
          <div className="bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-10 text-center">
            <Crown className="w-16 h-16 text-amber-500 mx-auto mb-4"/>
            <h2 className="text-2xl font-bold text-slate-100 mb-3">Premium Feature</h2>
            <p className="text-slate-300 mb-6">The Explanation Bank is available to Premium subscribers. Upgrade to review every question you got wrong with full explanations.</p>
            <button onClick={()=>upgrade(true)} className="w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900 mb-3">Upgrade to Premium -- $25/month</button>
            <button onClick={()=>setShowExplanations(false)} className="w-full bg-gray-100 text-slate-200 py-3 rounded-lg font-semibold hover:bg-gray-200">Back to Dashboard</button>
          </div>
        </div>
      );
    }
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          <button onClick={()=>setShowExplanations(false)} className="text-amber-600 hover:text-amber-800 mb-6 flex items-center font-medium">Back to Dashboard</button>
          <div className="flex items-center justify-between mb-2">
            <div>
              <h1 className="text-4xl font-bold text-slate-100">Explanation Bank</h1>
              <p className="text-slate-400 mt-1">Every question you got wrong -- with full explanations</p>
            </div>
            <span className="bg-amber-400 text-white px-3 py-1 rounded-full text-sm font-bold flex items-center gap-1"><Crown className="w-4 h-4"/>Premium</span>
          </div>

          <div className="flex gap-2 mt-6 mb-6">
            {[['all','All Missed'],['tests','From Tests'],['drill','From Drill']].map(([val,label])=>(
              <button key={val} onClick={()=>setExplainFilter(val)} className={`px-4 py-2 rounded-lg font-semibold text-sm transition-all ${explainFilter===val?'bg-amber-400 text-slate-900':'bg-slate-700 text-slate-300 hover:bg-slate-600 shadow'}`}>{label}</button>
            ))}
            <span className="ml-auto text-sm text-slate-400 flex items-center">{filtered.length} question{filtered.length!==1?'s':''} missed</span>
          </div>

          {filtered.length === 0 ? (
            <div className="bg-slate-800 rounded-2xl shadow-lg p-12 text-center">
              <div className="text-6xl mb-4">🎉</div>
              <h2 className="text-2xl font-bold text-slate-100 mb-3">{explainFilter === 'all' ? 'No missed questions yet' : 'No missed questions in this category'}</h2>
              <p className="text-slate-400 mb-6">{explainFilter === 'all' ? 'Complete practice tests or drill questions. Questions you get wrong will appear here with full explanations.' : 'Switch to a different filter or complete more questions.'}</p>
              <button onClick={()=>setShowExplanations(false)} className="bg-slate-800 text-white px-8 py-3 rounded-lg font-semibold hover:bg-slate-900">Go Practice</button>
            </div>
          ) : (
            <div className="space-y-6">
              {Object.entries(grouped).map(([label, questions]) => (
                <div key={label} className="bg-slate-800 rounded-2xl shadow-lg overflow-hidden border border-slate-700">
                  <div className={`px-6 py-4 flex items-center justify-between ${label.includes('Logical') || label.includes('Reading') ? 'bg-slate-800' : 'bg-slate-7000'}`}>
                    <div>
                        ...
                        <button onClick={()=>setExpandedQ(expandedQ===q.id?null:q.id)} className="w-full text-left">
                          <div className="flex items-start justify-between gap-3">
                            <div className="flex-1">
                              <p className="text-slate-400 text-xs mb-1 font-semibold uppercase tracking-wide">Stimulus</p>
                              <p className="text-slate-200 text-sm leading-relaxed line-clamp-2">{q.passage}</p>
                              <p className="text-slate-100 font-semibold mt-2 text-sm">{q.question}</p>
                            </div>
                            <div className={`flex-shrink-0 mt-1 w-6 h-6 rounded-full flex items-center justify-center text-white font-bold text-xs ${expandedQ===q.id?'bg-slate-800':'bg-gray-300'}`}>{expandedQ===q.id?'-':'+'}</div>
                          </div>
                        </button>
                        {expandedQ===q.id && (
                          <div className="mt-4 space-y-3">
                            <div className="bg-slate-700 rounded-lg p-4">
                              <p className="text-slate-200 text-sm leading-relaxed">{q.passage}</p>
                            </div>
                            ...
                            <div className="space-y-2">
                              {q.options && q.options.map((opt, idx) => {
                                ...
                                let cls = "bg-slate-700 border-slate-600";
                                if (isCorrect) cls = "bg-green-900 border-green-500";
                            ...
                            <div className="grid md:grid-cols-2 gap-3 mt-4">
                              <div className="bg-green-900 rounded-xl p-4 border border-green-700">
                                ...
                                <p className="text-slate-200 text-sm">This answer correctly identifies the core issue in the argument. It accurately reflects what the question demands without adding unsupported inferences or shifting the scope of the claim.</p>
                              </div>
                              <div className="bg-red-900 rounded-xl p-4 border border-red-700">
                                ...
                                <p className="text-slate-200 text-sm">This answer misidentifies the key issue. While plausible, it either overstates, understates, or shifts focus away from the specific logical concern the question targets.</p>
                              </div>
                            </div>
                          </div>
                        )}
  ...
  if (showProgress) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          <button onClick={()=>setShowProgress(false)} className="text-amber-600 hover:text-amber-800 mb-6 flex items-center font-medium">Back to Dashboard</button>
          <h1 className="text-4xl font-bold text-slate-100 mb-2">My Progress</h1>
          <p className="text-slate-400 mb-8">Your practice test history and performance breakdown</p>

          {totalTests === 0 ? (
            <div className="bg-slate-800 rounded-2xl shadow-lg p-12 text-center">
              <div className="text-6xl mb-4">📊</div>
              <h2 className="text-2xl font-bold text-slate-100 mb-3">No data yet</h2>
              <p className="text-slate-400 mb-6">Complete a practice test and mark it as completed to start tracking your progress.</p>
              <button onClick={()=>setShowProgress(false)} className="bg-slate-800 text-white px-8 py-3 rounded-lg font-semibold hover:bg-slate-900">Go Practice</button>
            </div>
          ) : (
            <>
              <div className="grid md:grid-cols-3 gap-6 mb-8">
                {[{label:"Tests Completed",value:totalTests,sub:"total across both sections"},{label:"Overall Average",value:overallAvg!==null?overallAvg+"%":"--",sub:"across all completed tests"},{label:"Weak Areas",value:sortedMissed.length>0?sortedMissed[0][0]:"None",sub:sortedMissed.length>0?`${sortedMissed[0][1]} questions missed`:"Keep it up!"}].map((s,i)=>(
                  <div key={i} className="bg-slate-800 rounded-xl shadow-lg p-6 text-center">
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                {[{label:"Logical Reasoning",color:"bg-slate-800",history:lrHistory,avg:lrAvg},{label:"Reading Comprehension",color:"bg-red-600",history:rcHistory,avg:rcAvg}].map(({label,color,history,avg})=>(
                  <div key={label} className="bg-slate-800 rounded-2xl shadow-lg p-6">
                    ...
                    {history.length===0 ? (
                      <p className="text-slate-500 text-sm text-center py-4">No tests completed yet</p>
                    ) : (
                      <div className="space-y-3 max-h-64 overflow-y-auto pr-1">
                        {history.map((e,i)=>(
                          <div key={i} className="flex items-center justify-between gap-3">
                            <div className="flex-shrink-0 w-28">
                              <p className="text-sm font-semibold text-slate-100">Test {e.testNum}</p>
                              <p className="text-xs text-slate-500">{e.date}</p>
                            </div>
                            <div className="flex-1"><ScoreBar pct={e.pct}/></div>
                            <div className="flex-shrink-0 text-right">
                              <p className="text-sm font-bold text-slate-100">{e.score}/{e.total}</p>
                            </div>
                          </div>
                        ))}
                      </div>
              ...
              {sortedMissed.length > 0 && (
                <div className="bg-slate-800 rounded-2xl shadow-lg p-6 mb-8">
                  <h2 className="text-lg font-bold text-slate-100 mb-2">Question Types Missed</h2>
                  <p className="text-sm text-slate-400 mb-5">Based on all completed tests -- focus your study on the types at the top</p>
                      ...
                      return (
                        <div key={i} className="flex items-center gap-4">
                          <div className="w-36 flex-shrink-0">
                            ...
                            <p className="text-xs text-slate-500">{count} question{count!==1?"s":""} missed</p>
                          </div>
                          <div className="flex-1 bg-slate-600 rounded-full h-3">
                            <div className={`h-3 rounded-full ${i===0?'bg-red-500':i===1?'bg-orange-400':i===2?'bg-amber-400':'bg-amber-400'}`} style={{width:`${pct}%`}}/>
                          </div>
                          <div className="w-8 text-right text-sm font-bold text-slate-300">{count}</div>
                        </div>
                      );
                    })}
                  </div>
                  <div className="mt-6 pt-4 border-t border-slate-700">
                    <p className="text-sm font-semibold text-slate-200 mb-2">Recommended next steps:</p>
                    <ul className="space-y-1">
                      {sortedMissed.slice(0,3).map(([type],i)=>(
                        <li key={i} className="text-sm text-slate-300 flex gap-2">
                          <span className="text-amber-500 font-bold">-</span>
                          <span>Review the <button onClick={()=>{setShowProgress(false);setShowLearn(true);setLearnSec(type==='Main Point'||type==='Inference'||type==='Function'||type==='Author Attitude'||type==='Structure'?'rc':'lr');setLearnArt(null);}} className="text-amber-600 font-semibold hover:underline">Learn section</button> for {type} questions</span>
                        </li>
                      ))}
                    </ul>
                  </div>
              ...
              <div className="bg-slate-800 rounded-2xl shadow-lg p-6">
                <h2 className="text-lg font-bold text-slate-100 mb-4">All Completed Tests</h2>
                {allHistory.length===0 ? <p className="text-slate-500 text-sm">No tests completed yet.</p> : (
                        ...
                        {allHistory.sort((a,b)=>new Date(b.date)-new Date(a.date)).map((e,i)=>{
                          ...
                          return (
                            <tr key={i} className="border-b border-slate-700 hover:bg-slate-700">
                              <td className="py-3"><span className={`px-2 py-0.5 rounded text-xs font-bold text-white ${e.section==='lr'?'bg-slate-800':'bg-red-500'}`}>{e.section.toUpperCase()}</span></td>
                              <td className="py-3 font-medium text-slate-100">Test {e.testNum}</td>
                              <td className="py-3 text-slate-200">{e.score}/{e.total}</td>
                              ...
                              <td className="py-3 text-slate-300">{topMissed?`${topMissed[0]} (${topMissed[1]})`:"--"}</td>
                            </tr>
                          );
                        })}
                      </tbody>
  ...
  if (showDrill) {
    ...
    const submitDrill = () => {
      ...
      setTotalQAnswered(n => {
        ...
        setAchievements(a => ({...a,
          firstQuestion: true,
          q100: next >= 100 ? true : a.q100,
          q1000: next >= 1000 ? true : a.q1000,
          allQuestions: next >= 2600 ? true : a.allQuestions,
        }));
        return next;
      });
      if (!isCorrect) {
      ...
      return (
        <div className="min-h-screen bg-slate-950 p-4 md:p-8">
          <div className="max-w-2xl mx-auto">
            <button onClick={()=>setShowDrill(false)} className="text-amber-600 hover:text-amber-800 mb-6 flex items-center font-medium">Back to Dashboard</button>
            <div className="text-center mb-10">
              <h1 className="text-4xl font-bold text-slate-100 mb-3">Drill Mode</h1>
              <p className="text-slate-300 text-lg">Random questions from the full bank. Answer as many as you like -- no time limit, no pressure.</p>
              {!isPremium && <p className="text-sm text-amber-600 bg-slate-700 border border-amber-200 rounded-lg px-4 py-2 mt-4 inline-block">Upgrade to Premium to unlock explanations after each question</p>}
            </div>
            <div className="grid gap-4">
              {[{type:'lr',label:'Logical Reasoning',desc:'Random LR questions from the full bank',color:'bg-slate-800'},{type:'rc',label:'Reading Comprehension',desc:'Random RC questions from the full bank',color:'bg-red-600'},{type:'mixed',label:'Mixed',desc:'Random mix of LR and RC questions',color:'bg-gradient-to-br from-slate-700 to-amber-600'}].map(({type,label,desc,color})=>(
      ...
      <div className="min-h-screen bg-slate-950 p-4 md:p-8">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center justify-between mb-6">
            <button onClick={()=>{setDrillType(null);setDrillQ(null);setDrillSel(null);setDrillShowRes(false);}} className="text-amber-600 hover:text-amber-800 font-medium">Back to Drill Menu</button>
            {drillScore.total > 0 && (
              <div className="flex items-center gap-4 bg-slate-800 rounded-xl shadow px-5 py-2 border border-slate-700">
                <span className="text-sm text-slate-400">{drillScore.total} answered</span>
          ...
          <div className="bg-slate-800 rounded-xl shadow-lg p-6 md:p-8">
            <div className="flex items-center justify-between mb-4">
              <span className={`text-xs font-bold px-3 py-1 rounded-full text-white ${drillQ.sourceType === 'lr' ? 'bg-slate-800' : 'bg-red-600'}`}>{drillQ.sourceType === 'lr' ? 'Logical Reasoning' : 'Reading Comprehension'}</span>
              <span className="text-sm text-slate-500">Question {drillScore.total + (drillShowRes ? 0 : 1)}</span>
            </div>

            <div className="bg-slate-700 rounded-lg p-5 mb-5"><p className="text-slate-100 leading-relaxed">{drillQ.passage}</p></div>
            <h3 className="text-lg font-semibold text-slate-100 mb-4">{drillQ.question}</h3>

            <div className="space-y-3 mb-6">
              {drillQ.options.map((opt, idx) => {
                const sel = drillSel === idx; const cor = idx === drillQ.correct;
                let cls = "bg-slate-700 hover:bg-slate-600 border-slate-600"; let icon = null;
                if (drillShowRes) {
                  if (cor) { cls = "bg-green-900 border-green-500"; icon = <CheckCircle className="w-5 h-5 text-green-600"/>; }
                  else if (sel) { cls = "bg-red-900 border-red-500"; icon = <XCircle className="w-5 h-5 text-red-600"/>; }
                } else if (sel) { cls = "bg-slate-700 border-amber-500"; }
                return (
                  <button key={idx} onClick={()=>{ if (!drillShowRes) setDrillSel(idx); }} disabled={drillShowRes} className={`w-full text-left p-4 rounded-lg border-2 flex items-center justify-between ${cls}`}>
                    <span className="text-slate-100">{opt}</span>{icon}
                  </button>
                );
              })}
            </div>
            ...
            {drillShowRes && (
              <div className={`rounded-lg p-5 mb-6 border-2 ${isPremium ? 'bg-slate-700 border-slate-500' : 'bg-slate-700 border-amber-300'}`}>
                {isPremium ? (
                  <>
                    <div className="flex items-center gap-2 mb-3"><Crown className="w-5 h-5 text-amber-500"/><span className="font-semibold text-slate-100">Premium Explanation</span></div>
                    <div className="p-4 bg-green-900 rounded-lg border border-green-700 mb-3">
                      ...
                      <p className="text-slate-200 text-sm">This answer correctly identifies the core issue. It accurately reflects what the question is asking without adding unsupported inferences or misrepresenting the scope of the claim.</p>
                    </div>
                    {drillSel !== drillQ.correct && (
                      <div className="p-4 bg-red-900 rounded-lg border border-red-700">
                        ...
                        <p className="text-slate-200 text-sm">This answer misidentifies the key issue. While it may seem plausible, it either overstates, understates, or shifts focus away from the primary concern the question targets.</p>
                      </div>
                    )}
                  </>
                ) : (
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="font-semibold text-slate-100 mb-1 flex items-center gap-2">Detailed Explanations <Crown className="w-4 h-4 text-amber-500"/></p>
                      <p className="text-slate-300 text-sm mb-3">Upgrade to Premium to see why each answer is right or wrong after every question.</p>
                      <button onClick={()=>upgrade(true)} className="bg-slate-800 text-white px-5 py-2 rounded-lg font-semibold text-sm hover:bg-slate-900">Upgrade to Premium - $25/month</button>
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex justify-between">
              {!drillShowRes ? (
                <button onClick={submitDrill} disabled={drillSel === null} className="w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900 disabled:opacity-40 disabled:cursor-not-allowed">Submit Answer</button>
              ) : (
                <button onClick={nextDrillQ} className="w-full bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900 flex items-center justify-center gap-2">Next Question <ChevronRight className="w-5 h-5"/></button>
              )}
            </div>
          </div>
        </div>
  ...
  if (showAchievements) {
    const BADGES = [
      { key: 'firstQuestion', icon: '🎯', title: 'First Step', desc: 'Answer your first question', unlocked: achievements.firstQuestion },
      { key: 'firstTest', icon: '📋', title: 'Test Taker', desc: 'Complete your first practice test', unlocked: achievements.firstTest },
      { key: 'firstArticle', icon: '📖', title: 'Student of the Game', desc: 'Read your first Learn article', unlocked: achievements.firstArticle },
      { key: 'q100', icon: '💯', title: 'Century', desc: 'Answer 100 questions', unlocked: achievements.q100 },
      { key: 'q1000', icon: '🔥', title: 'On Fire', desc: 'Answer 1,000 questions', unlocked: achievements.q1000 },
      { key: 'allQuestions', icon: '👑', title: 'The Closer', desc: 'Answer all 2,600 questions', unlocked: achievements.allQuestions },
      { key: 'timedLSAT', icon: '⏱️', title: 'Full Send', desc: 'Complete a full timed LSAT', unlocked: achievements.timedLSAT },
    ];
    const unlocked = BADGES.filter(b => b.unlocked).length;
    return (
      <div className="min-h-screen bg-slate-950 p-4 md:p-8">
        <div className="max-w-3xl mx-auto">
          <button onClick={()=>setShowAchievements(false)} className="text-amber-400 hover:text-amber-300 mb-6 flex items-center font-medium">← Back to Dashboard</button>
          <div className="text-center mb-10">
            <h1 className="text-4xl font-bold text-white mb-2">Achievements</h1>
            <p className="text-slate-400">{unlocked} of {BADGES.length} unlocked</p>
            <div className="mt-4 h-2 bg-slate-700 rounded-full max-w-xs mx-auto">
              <div className="h-2 bg-amber-400 rounded-full transition-all" style={{width: `${unlocked/BADGES.length*100}%`}}/>
            </div>
          </div>
          <div className="grid md:grid-cols-2 gap-4">
            {BADGES.map(badge => (
              <div key={badge.key} className={`rounded-2xl p-6 border-2 flex items-center gap-5 transition-all ${badge.unlocked ? 'bg-slate-800 border-amber-400 shadow-lg' : 'bg-slate-900 border-slate-700 opacity-50'}`}>
                <div className={`w-16 h-16 rounded-full flex items-center justify-center text-3xl flex-shrink-0 ${badge.unlocked ? 'bg-amber-400' : 'bg-slate-700'}`}>
                  {badge.unlocked ? badge.icon : '🔒'}
                </div>
                <div>
                  <h3 className={`text-lg font-bold mb-1 ${badge.unlocked ? 'text-white' : 'text-slate-500'}`}>{badge.title}</h3>
                  <p className={`text-sm ${badge.unlocked ? 'text-slate-300' : 'text-slate-600'}`}>{badge.desc}</p>
                  {badge.unlocked && <span className="text-xs text-amber-400 font-semibold mt-1 block">✓ Unlocked</span>}
                </div>
              </div>
            ))}
          </div>
          {unlocked === BADGES.length && (
            <div className="mt-8 bg-amber-400 rounded-2xl p-6 text-center">
              <p className="text-3xl mb-2">🎉</p>
              <h2 className="text-2xl font-bold text-slate-900 mb-1">All achievements unlocked!</h2>
              <p className="text-slate-800">You are the Specter. Harvey would be proud.</p>
            </div>
          )}
        </div>
      </div>
  ...
  if (!section) {
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-8">
        <div className="max-w-4xl mx-auto">
          <div className="bg-slate-800 rounded-xl shadow-lg p-4 mb-8 flex flex-wrap justify-between items-center gap-3">
            <div className="flex items-center gap-3"><User className="w-8 h-8 text-amber-600"/><div><p className="font-semibold text-slate-100">{user.username}</p><p className="text-sm text-slate-300">{user.email}</p></div></div>
            <div className="flex items-center gap-2 flex-wrap">
              {user.isPremium&&<span className="bg-amber-400 text-slate-900 px-3 py-1 rounded-full text-sm font-semibold flex items-center gap-1"><Crown className="w-4 h-4"/>Premium</span>}
              <button onClick={()=>{setShowExplanations(true);setExplainFilter('all');}} className="flex items-center gap-2 px-4 py-2 bg-amber-400 text-slate-900 border-2 border-amber-400 rounded-lg hover:bg-amber-300 text-sm font-semibold"><Crown className="w-4 h-4"/>Explanation Bank</button>
              <button onClick={()=>setShowProgress(true)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold"><CheckCircle className="w-4 h-4"/>My Progress</button>
              {user.isAdmin && <button onClick={()=>{setShowAdmin(true);setAdminSearch('');setAdminResult(null);setAdminMsg('');}} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold"><User className="w-4 h-4"/>Admin Panel</button>}
              <button onClick={()=>setShowAccount(true)} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold"><User className="w-4 h-4"/>My Account</button>
              <button onClick={()=>setShowTerms(true)} className="px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold">Terms</button><button onClick={()=>setShowPrivacy(true)} className="px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold">Privacy</button><button onClick={()=>setShowAchievements(true)} className="px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold">🏆 Achievements</button>
              <button onClick={doLogout} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-amber-400 border-2 border-amber-400 rounded-lg hover:bg-slate-800 text-sm font-semibold"><LogOut className="w-4 h-4"/>Logout</button>
            </div>
          </div>
          <div className="text-center mb-8"><h1 className="text-4xl font-bold text-slate-100 mb-3">Specter LSAT Prep</h1><p className="text-slate-400">2,600 questions per section -- 100 practice tests of 26 questions each</p></div>
          <div className="grid md:grid-cols-2 gap-6 mb-6">
            <button onClick={()=>{setShowLearn(true);setLearnSec(null);setLearnArt(null);}} className="bg-slate-900 border-2 border-amber-400 rounded-xl shadow-lg p-8 hover:bg-slate-800 hover:shadow-xl text-white text-center"><div className="w-12 h-12 bg-amber-400 rounded-full flex items-center justify-center mx-auto mb-4"><BookOpen className="w-6 h-6 text-slate-900"/></div><h2 className="text-xl font-bold mb-2">Learn</h2><p className="text-slate-300 text-sm mb-4">Strategy guides for every question type</p><div className="text-amber-400 text-sm font-semibold">3 Sections -- 25 Articles -- Free for all users</div></button>
            <button onClick={()=>canAccess?startTimed():setShowAccess(true)} className="bg-slate-900 border-2 border-amber-400 rounded-xl shadow-lg p-8 hover:bg-slate-800 hover:shadow-xl text-white text-center relative">{!canAccess&&<Lock className="w-5 h-5 text-amber-400 opacity-70 absolute top-4 right-4"/>}<div className="w-12 h-12 bg-amber-400 rounded-full flex items-center justify-center mx-auto mb-4"><List className="w-6 h-6 text-slate-900"/></div><h2 className="text-xl font-bold mb-2">Timed Full Test</h2><p className="text-slate-300 text-sm mb-4">Real LSAT conditions, 3 sections</p><div className="text-amber-400 text-sm font-semibold">78 Questions -- 35 min each</div></button>
          </div>
          <div className="grid md:grid-cols-3 gap-6 mb-6">
            <button onClick={()=>{setShowDrill(true);setDrillType(null);setDrillQ(null);setDrillSel(null);setDrillShowRes(false);setDrillScore({correct:0,total:0});setDrillHistory([]);}} className="bg-slate-900 border-2 border-amber-400 rounded-xl shadow-lg p-8 hover:bg-slate-800 hover:shadow-xl text-white text-center col-span-full">
              <div className="w-12 h-12 bg-amber-400 rounded-full flex items-center justify-center mx-auto mb-4"><Brain className="w-6 h-6 text-slate-900"/></div>
              <h2 className="text-xl font-bold mb-2">Drill Mode</h2>
              <p className="text-slate-300 text-sm mb-4">Random questions from the full bank -- free for everyone, explanations for Premium</p>
              <div className="text-amber-400 text-sm font-semibold">LR -- RC -- Mixed -- Free for all users</div>
            </button>
          </div>
          <div className="grid md:grid-cols-2 gap-6">
            {Object.entries(SECTIONS).map(([key,sec])=>{
              const Icon=sec.icon;
              return <button key={key} onClick={()=>canAccess?setSection(key):setShowAccess(true)} className="bg-slate-900 border-2 border-amber-400 rounded-xl shadow-lg p-8 hover:bg-slate-800 hover:shadow-xl text-white text-center relative">{!canAccess&&<Lock className="w-5 h-5 text-amber-400 opacity-70 absolute top-4 right-4"/>}<Icon className="w-12 h-12 text-amber-400 mx-auto mb-4"/><h2 className="text-xl font-bold mb-2">{sec.name}</h2><p className="text-slate-300 text-sm mb-4">{sec.desc}</p><div className="text-amber-400 text-sm font-semibold">1,300 Questions -- {sec.count} Tests</div>{!canAccess&&<div className="mt-3 text-xs text-amber-300 font-semibold">Subscription Required</div>}</button>;
            })}
          </div>
        </div>
          ...
          <div className="bg-slate-800 rounded-2xl max-w-lg w-full p-8 shadow-2xl">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-1">Manage user subscriptions</p>
              </div>
              <button onClick={()=>setShowAdmin(false)} className="text-slate-500 hover:text-slate-300 text-2xl font-bold leading-none">x</button>
            </div>

            <div className="mb-6">
              <label className="block text-sm font-semibold text-slate-200 mb-2">Look Up User by Username</label>
              <div className="flex gap-2">
                <input type="text" value={adminSearch} onChange={e=>setAdminSearch(e.target.value)} placeholder="Enter username or email" className="flex-1 px-4 py-2 border-2 border-slate-600 rounded-lg focus:border-amber-500 focus:outline-none"/>
                <button onClick={async ()=>{
                  setAdminResult(null); setAdminMsg('');
                  if (!adminSearch.trim()) return setAdminMsg('Please enter a username or email.');
                  try {
                    // Search Supabase by username first
                    let found = await sb.getUser(adminSearch.trim());
                    // If not found by username, try searching by email
                    if (!found) {
                      try {
                        const r = await fetch(`${SUPABASE_URL}/rest/v1/users?email=eq.${encodeURIComponent(adminSearch.trim())}&limit=1`, {
                          headers: { apikey: SUPABASE_ANON_KEY, Authorization: `Bearer ${SUPABASE_ANON_KEY}` }
                        });
                        const data = await r.json();
                    ...
                    if (!found) return setAdminMsg('User not found in database.');
                    setAdminResult({ username: found.username, email: found.email, isPremium: found.is_premium, hasTestAccess: found.has_test_access, createdAt: found.created_at });
                  } catch(e) { setAdminMsg('Error looking up user.'); }
                }} className="bg-slate-800 text-white px-5 py-2 rounded-lg font-semibold hover:bg-slate-900">Search</button>
              </div>
              {adminMsg && <p className={`mt-2 text-sm font-semibold ${adminMsg.includes('not found') || adminMsg.includes('Error') ? 'text-red-600' : 'text-green-600'}`}>{adminMsg}</p>}
            </div>

            {adminResult && (
              <div className="border-2 border-slate-600 rounded-xl p-5">
                <div className="grid grid-cols-2 gap-3 mb-5">
                  {[['Username', adminResult.username], ['Email', adminResult.email || 'Not provided'], ['Member Since', adminResult.createdAt ? new Date(adminResult.createdAt).toLocaleDateString('en-US', {month:'short', day:'numeric', year:'numeric'}) : 'N/A']].map(([l,v])=>(
                    <div key={l} className="bg-slate-700 rounded-lg p-3">
                      <p className="text-xs text-slate-400 font-semibold uppercase tracking-wide mb-1">{l}</p>
                      <p className="text-slate-100 font-medium text-sm">{v}</p>
                    </div>
                  ))}
                  <div className="bg-slate-700 rounded-lg p-3">
                    <p className="text-xs text-slate-400 font-semibold uppercase tracking-wide mb-1">Current Plan</p>
                    {adminResult.isPremium ? <span className="bg-amber-400 text-white text-xs px-2 py-0.5 rounded-full font-bold">Premium</span>
                    : adminResult.hasTestAccess ? <span className="bg-slate-700 text-amber-700 text-xs px-2 py-0.5 rounded-full font-bold">Test Access</span>
                    : <span className="bg-gray-100 text-slate-300 text-xs px-2 py-0.5 rounded-full font-bold">Free</span>}
                  </div>
                </div>

                <p className="text-sm font-semibold text-slate-200 mb-3">Update Subscription</p>
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <button onClick={async ()=>{
                    try {
                      await sb.upsertUser({ username: adminResult.username, email: adminResult.email, password: '', is_premium: false, has_test_access: true, created_at: adminResult.createdAt });
                      setAdminResult({...adminResult, hasTestAccess: true, isPremium: false});
                      setAdminMsg('Updated to Test Access successfully.');
                    } catch(e) { setAdminMsg('Error updating user.'); }
                  }} className="bg-gray-800 text-white py-2 rounded-lg font-semibold text-sm hover:bg-gray-900">Test Access</button>
                  <button onClick={async ()=>{
                    try {
                      await sb.upsertUser({ username: adminResult.username, email: adminResult.email, password: '', is_premium: true, has_test_access: true, created_at: adminResult.createdAt });
                      setAdminResult({...adminResult, hasTestAccess: true, isPremium: true});
                      setAdminMsg('Upgraded to Premium successfully.');
                    } catch(e) { setAdminMsg('Error updating user.'); }
                  }} className="bg-slate-800 text-white py-2 rounded-lg font-semibold text-sm hover:bg-slate-900">Premium</button>
                  <button onClick={async ()=>{
                    try {
                      await sb.upsertUser({ username: adminResult.username, email: adminResult.email, password: '', is_premium: false, has_test_access: false, created_at: adminResult.createdAt });
                      setAdminResult({...adminResult, hasTestAccess: false, isPremium: false});
                      setAdminMsg('Downgraded to Free successfully.');
                    } catch(e) { setAdminMsg('Error updating user.'); }
                  }} className="bg-red-100 text-red-700 py-2 rounded-lg font-semibold text-sm hover:bg-red-200">Downgrade to Free</button>
                </div>
                <p className="text-xs text-slate-500">Changes update Supabase immediately. The user must log out and back in to see their updated plan.</p>
              </div>
            )}
          </div>
        </div>
      )}
      {showPrivacy&&<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"><div className="bg-slate-800 rounded-2xl max-w-lg w-full p-8 shadow-2xl"><div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold text-slate-100">Privacy Policy</h2><button onClick={()=>setShowPrivacy(false)} className="text-slate-500 hover:text-slate-300 text-2xl font-bold leading-none">x</button></div><div className="text-sm text-slate-200 space-y-4 max-h-96 overflow-y-auto pr-2"><p className="text-xs text-slate-500">Last updated: April 2026</p><div><p className="font-bold text-slate-100 mb-1">1. Information We Collect</p><p>We collect your username, email address, and password when you create an account, along with your practice scores and progress data.</p></div><div><p className="font-bold text-slate-100 mb-1">2. How We Use Your Information</p><p>We use your information to operate Specter LSAT Prep, process payments through Stripe, and communicate about your account. We do not sell your personal information.</p></div><div><p className="font-bold text-slate-100 mb-1">3. Data Storage</p><p>Account data is stored securely in Supabase. Payments are processed by Stripe. We do not store credit card information.</p></div><div><p className="font-bold text-slate-100 mb-1">4. Your Rights</p><p>You may request deletion of your account by emailing specterlsatprep@gmail.com. We process deletion requests within 30 days.</p></div><div><p className="font-bold text-slate-100 mb-1">5. Contact</p><p>specterlsatprep@gmail.com</p></div></div><button onClick={()=>setShowPrivacy(false)} className="w-full mt-6 bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900">Close</button></div></div>}
        {showTerms&&<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"><div className="bg-slate-800 rounded-2xl max-w-md w-full p-8 shadow-2xl"><h2 className="text-2xl font-bold mb-4">Terms of Service</h2><div className="text-sm text-slate-200 space-y-3 max-h-64 overflow-y-auto"><p><strong>1. Agreement.</strong> By using Specter LSAT Prep, you agree to these Terms of Service.</p><p><strong>2. Subscriptions.</strong> Test Access ($20/mo) provides access to all practice questions and tests. Premium ($25/mo) adds detailed answer explanations. Subscriptions renew monthly until cancelled.</p><p><strong>3. Refunds.</strong> Refunds may be provided under certain conditions. Contact us at specterlsatprep@gmail.com within 7 days of purchase to request a refund.</p><p><strong>4. Cancellations.</strong> You may cancel your subscription at any time through the Stripe customer portal. Access continues until the end of your billing period.</p><p><strong>5. Account Responsibility.</strong> You are responsible for maintaining the confidentiality of your account credentials. You may not share your account with others.</p><p><strong>6. Intellectual Property.</strong> All content on Specter LSAT Prep is proprietary. You may not reproduce or distribute it without permission.</p><p><strong>7. Disclaimer.</strong> Specter LSAT Prep is an independent test preparation service. We are not affiliated with or endorsed by LSAC.</p><p><strong>8. Governing Law.</strong> These terms are governed by the laws of Florida, USA.</p><p><strong>9. Contact.</strong> specterlsatprep@gmail.com</p></div><button onClick={()=>setShowTerms(false)} className="w-full mt-6 bg-slate-800 text-white py-3 rounded-lg font-semibold hover:bg-slate-900">Close</button></div></div>}
        {showAccount&&<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4"><div className="bg-slate-800 rounded-2xl max-w-md w-full p-8 shadow-2xl"><div className="flex justify-between items-center mb-6"><h2 className="text-2xl font-bold text-slate-100">My Account</h2><button onClick={()=>{setShowAccount(false);setShowCancel(false);}} className="text-slate-500 hover:text-slate-300 text-2xl font-bold">x</button></div>{!showCancel?(<><div className="space-y-3 mb-6">{[["Username",user.username],["Email",user.email||"Not provided"],["Member Since",user.createdAt?new Date(user.createdAt).toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"}):"N/A"],["Tests Completed",`${Object.keys(completed).length} practice test${Object.keys(completed).length!==1?"s":""}`]].map(([l,v])=><div key={l} className="bg-slate-700 rounded-xl p-4"><p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-1">{l}</p><p className="text-slate-100 font-medium">{v}</p></div>)}<div className="bg-slate-700 rounded-xl p-4"><p className="text-xs text-slate-400 uppercase tracking-wide font-semibold mb-1">Subscription</p>{user.isAdmin?<span className="bg-gray-800 text-white text-xs px-3 py-1 rounded-full font-bold">Admin</span>:user.isPremium?<span className="bg-amber-400 text-white text-xs px-3 py-1 rounded-full font-bold">Premium - $25/month</span>:user.hasTestAccess?<span className="bg-slate-700 text-amber-700 text-xs px-3 py-1 rounded-full font-bold">Test Access - $20/month</span>:<span className="bg-gray-100 text-slate-300 text-xs px-3 py-1 rounded-full font-bold">Free</span>}</div></div><div className="space-y-3">{(user.isPremium||user.hasTestAccess)&&!user.isAdmin&&<button onClick={()=>setShowCancel(true)} className="w-full py-3 rounded-xl font-semibold border-2 border-red-300 text-red-600 hover:bg-red-50">Cancel Subscription</button>}<button onClick={()=>{setShowAccount(false);doLogout();}} className="w-full py-3 rounded-xl font-semibold bg-gray-100 text-slate-200 hover:bg-gray-200 flex items-center justify-center gap-2"><LogOut className="w-4 h-4"/>Sign Out</button></div></>):(<div className="text-center"><h3 className="text-xl font-bold text-slate-100 mb-2">Cancel Your Subscription?</h3><p className="text-slate-300 mb-2">You will lose access at the end of your current billing period.</p><p className="text-sm text-slate-400 mb-6">You can cancel directly through Stripe's secure customer portal. No need to contact us.</p><div className="space-y-3"><a href="https://billing.stripe.com/p/login/3cI7sE2wgbsg9uveXJ7EQ00" target="_blank" rel="noopener noreferrer" className="w-full py-3 rounded-xl font-semibold bg-red-600 text-white hover:bg-red-700 block text-center">Manage Subscription in Stripe</a><button onClick={()=>setShowCancel(false)} className="w-full py-3 rounded-xl font-semibold bg-gray-100 text-slate-200 hover:bg-gray-200">Keep My Subscription</button></div></div>)}</div></div>}
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-950 p-8">
        <div className="max-w-5xl mx-auto">
          <button onClick={()=>setSection(null)} className="text-amber-600 hover:text-amber-800 mb-6 flex items-center">Back to Sections</button>
          <div className="bg-slate-800 rounded-xl shadow-lg p-8 mb-8"><div className="flex items-center gap-4"><Icon className="w-10 h-10 text-amber-600"/><div><h2 className="text-3xl font-bold text-slate-100">{sec.name}</h2><p className="text-slate-300">Select a practice test -- 26 questions each</p></div></div></div>
          <div className="grid md:grid-cols-3 gap-4">
            {Array.from({length:sec.count},(_,i)=>{
              const tk=`${section}-${i}`; const done=completed[tk];
              return <button key={i} onClick={()=>{setTestIdx(i);setQIdx(0);setAnswers({});setShowRes(false);setSelAns(null);}} className="bg-slate-700 rounded-lg shadow-md p-6 hover:shadow-lg hover:bg-slate-700 border-2 border-transparent hover:border-amber-400 relative">{done&&<CheckCircle className="w-6 h-6 text-green-600 absolute top-2 right-2"/>}<List className="w-8 h-8 text-amber-600 mx-auto mb-3"/><h3 className="text-lg font-bold text-slate-100">Practice Test {i+1}</h3><p className="text-sm text-slate-300 mt-1">Questions {i*26+1}--{(i+1)*26}</p>{done&&<p className="text-xs text-green-600 mt-2 font-semibold">Completed</p>}</button>;
            })}
          </div>
        </div>
      </div>
  ...
  if (!q) return <div className="p-8 text-center text-slate-300">Loading...</div>;
  ...
  return (
    <div className="min-h-screen bg-slate-950 p-4 md:p-8">
      <div className="max-w-4xl mx-auto">
        <div className="bg-slate-800 rounded-xl shadow-lg p-6 md:p-8">
          <button onClick={()=>{setTestIdx(null);setQIdx(0);setAnswers({});setShowRes(false);setSelAns(null);}} className="text-amber-600 hover:text-amber-800 mb-4 flex items-center">Back to Practice Tests</button>
          <div className="flex justify-between items-center mb-3">
            <div><h2 className="text-2xl font-bold text-slate-100">{SECS[section].name}</h2><p className="text-sm text-slate-300">Practice Test {testIdx+1}</p></div>
            <span className="text-slate-300">Question {qIdx+1} of {qs.length}</span>
          </div>
          <div className="w-full bg-slate-600 rounded-full h-2 mb-6"><div className="bg-slate-800 h-2 rounded-full" style={{width:`${prog}%`}}/></div>
          <div className="bg-slate-700 rounded-lg p-6 mb-6"><p className="text-slate-100 leading-relaxed">{q.passage}</p></div>
          <h3 className="text-lg font-semibold text-slate-100 mb-4">{q.question}</h3>
          <div className="space-y-3 mb-6">
            {q.options.map((opt,idx)=>{
              const s=selAns===idx; const c=idx===q.correct; let cls="bg-slate-700 hover:bg-slate-600 border-slate-600"; let ico=null;
              if(showRes){if(c){cls="bg-green-900 border-green-500";ico=<CheckCircle className="w-5 h-5 text-green-600"/>;}else if(s){cls="bg-red-900 border-red-500";ico=<XCircle className="w-5 h-5 text-red-600"/>;}}else if(s){cls="bg-slate-700 border-amber-500";}
              return <button key={idx} onClick={()=>{if(!showRes){setSelAns(idx);setAnswers(a=>({...a,[qIdx]:idx}));}}} disabled={showRes} className={`w-full text-left p-4 rounded-lg border-2 flex items-center justify-between ${cls}`}><span className="text-slate-100">{opt}</span>{ico}</button>;
            })}
          </div>

          {showRes&&(
            <div className={`rounded-lg p-6 mb-6 border-2 ${user.isPremium&&isDone?"bg-slate-700 border-slate-500":"bg-slate-700 border-amber-300"}`}>
              {user.isPremium&&isDone?(<>
                <div className="flex items-center gap-2 mb-4"><Crown className="w-5 h-5 text-amber-500"/><span className="font-semibold text-slate-100">Premium Explanations</span></div>
                <div className="p-4 bg-green-900 rounded-lg border border-green-700 mb-3"><div className="flex items-center gap-2 mb-2"><CheckCircle className="w-5 h-5 text-green-600"/><span className="font-semibold text-green-800">Why the correct answer is right:</span></div><p className="text-slate-200 text-sm">This answer correctly identifies the core issue. It accurately reflects what the question is asking without adding unsupported inferences or misrepresenting the scope of the claim.</p></div>
                {selAns!==q.correct&&<div className="p-4 bg-red-900 rounded-lg border border-red-700"><div className="flex items-center gap-2 mb-2"><XCircle className="w-5 h-5 text-red-600"/><span className="font-semibold text-red-800">Why your answer was incorrect:</span></div><p className="text-slate-200 text-sm">This answer misidentifies the key issue. While it may seem plausible, it either overstates, understates, or shifts focus away from the primary concern the question targets.</p></div>}
              </>):(
                <div className="flex items-start gap-4"><Lock className="w-6 h-6 text-amber-600 flex-shrink-0 mt-1"/>
                  <div><h4 className="font-semibold text-slate-100 mb-2 flex items-center gap-2">Detailed Explanations<Crown className="w-5 h-5 text-amber-500"/></h4>
                  <p className="text-slate-200 text-sm mb-3">{!isDone?"Complete all 26 questions to unlock explanations with Premium.":"Upgrade to Premium to access detailed explanations."}</p>
                  {isDone&&<button onClick={()=>setShowPaywall(true)} className="bg-slate-800 text-white px-6 py-2 rounded-lg font-semibold text-sm hover:bg-slate-900">Unlock Explanations -- $25/month</button>}</div>
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex flex-wrap gap-3 justify-between">
            <button onClick={()=>{if(qIdx>0){setQIdx(q=>q-1);setSelAns(answers[qIdx-1]??null);setShowRes(false);}}} disabled={qIdx===0} className="px-6 py-2 bg-slate-700 rounded-lg hover:bg-slate-600 disabled:opacity-50">Previous</button>
            <div className="flex gap-3">
              {!showRes&&selAns!==null&&<button onClick={()=>setShowRes(true)} className="px-6 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900">Submit Answer</button>}
              {showRes&&qIdx<qs.length-1&&<button onClick={()=>{setQIdx(q=>q+1);setSelAns(answers[qIdx+1]??null);setShowRes(false);}} className="px-6 py-2 bg-slate-800 text-white rounded-lg hover:bg-slate-900 flex items-center">Next<ChevronRight className="w-4 h-4 ml-1"/></button>}
          ...
          {qIdx===qs.length-1&&showRes&&allDone&&(
            <div className="mt-6 p-6 bg-gradient-to-r from-slate-800 to-slate-700 rounded-lg text-white text-center">
              ...
              {!isDone&&<button onClick={()=>{
                ...
                }} className="mt-4 bg-slate-700 text-amber-400 px-6 py-2 rounded-lg font-semibold hover:bg-slate-600">Mark as Completed</button>}
              {isDone&&!user.isPremium&&<button onClick={()=>setShowPaywall(true)} className="mt-4 bg-amber-400 text-white px-6 py-2 rounded-lg font-semibold flex items-center gap-2 mx-auto"><Crown className="w-5 h-5"/>Unlock Explanations</button>}
            </div>
          )}
        </div>
      </div>
