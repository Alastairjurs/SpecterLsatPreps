...
export default function SpecterLSATApp() {
  ...
  useEffect(() => {
    ...
    if (session?.user) {
      ...
      setUser(u);
      setShowHome(false);
      if (u.userId && session.token) {
        sb.getProgress(u.userId, session.token).then(progress => {
          if (progress) {
            try { if (progress.completed) setCompleted(JSON.parse(progress.completed)); } catch(e) {}
            try { if (progress.test_history) setTestHistory(JSON.parse(progress.test_history)); } catch(e) {}
            try { if (progress.drill_missed) setDrillMissed(JSON.parse(progress.drill_missed)); } catch(e) {}
          }
        }).catch(() => {});
      }
    }
  ...
  useEffect(() => {
    if (user?.userId && user?.token && Object.keys(completed).length > 0) {
      sb.upsertProgress({ user_id: user.userId, completed: JSON.stringify(completed) }, user.token).catch(() => {});
    }
  }, [completed, user]);
  useEffect(() => {
    if (user?.userId && user?.token && Object.keys(testHistory).length > 0) {
      sb.upsertProgress({ user_id: user.userId, test_history: JSON.stringify(testHistory) }, user.token).catch(() => {});
    }
  }, [testHistory, user]);
  useEffect(() => {
    if (user?.userId && user?.token && drillMissed.length > 0) {
      sb.upsertProgress({ user_id: user.userId, drill_missed: JSON.stringify(drillMissed) }, user.token).catch(() => {});
    }
  }, [drillMissed, user]);
  ...
  const doLogin = async () => {
    ...
    // Admin accounts -- bypass Supabase
    const admins = [
    ...
    if (admin) {
      ...
      sb.auth.saveSession({ user: u });
      setUser(u); setShowHome(false); setForm({ username: "", email: "", password: "" }); return;
    }

    try {
      // Sign in with Supabase Auth using username as email prefix
      const loginEmail = username.includes('@') ? username : `${username}@specterlsat.app`;
      const data = await sb.auth.signIn(loginEmail, password);
      ...
      const profile = await sb.getProfile(userId, token);
      const u = {
        username: profile?.username || username,
        email: data.user.email,
        isPremium: profile?.is_premium || false,
        hasTestAccess: profile?.has_test_access || false,
      ...
      // Load progress data
      const progress = await sb.getProgress(userId, token);
      if (progress) {
        try { if (progress.completed) setCompleted(JSON.parse(progress.completed)); } catch(e) {}
        try { if (progress.test_history) setTestHistory(JSON.parse(progress.test_history)); } catch(e) {}
        try { if (progress.drill_missed) setDrillMissed(JSON.parse(progress.drill_missed)); } catch(e) {}
      ...
      sb.auth.saveSession({ user: u, token, userId });
  ...
  const doSignUp = async () => {
    ...
    try {
      const data = await sb.auth.signUp(email, password, username);

      if (data.error) return alert(data.error.message || "Error creating account.");
  ...
  const doLogout = async () => {
    const session = sb.auth.getSession();
    if (session?.token) {
      try { await sb.auth.signOut(session.token); } catch(e) {}
    }
          ...
          <div className="bg-white rounded-2xl max-w-lg w-full p-8 shadow-2xl">
            ...
            <div className="mb-6">
              ...
              <div className="flex gap-2">
                <input type="text" value={adminSearch} onChange={e=>setAdminSearch(e.target.value)} placeholder="Enter exact username" className="flex-1 px-4 py-2 border-2 border-gray-200 rounded-lg focus:border-indigo-500 focus:outline-none"/>
                <button onClick={async ()=>{
                  setAdminResult(null); setAdminMsg('');
                  if (!adminSearch.trim()) return setAdminMsg('Please enter a username.');
                  try {
                    const r = await ls.get('u_' + adminSearch.trim());
                    if (!r || !r.value) return setAdminMsg('User not found. Make sure the username is exact.');
                    setAdminResult(JSON.parse(r.value));
            ...
            {adminResult && (
              <div className="border-2 border-gray-200 rounded-xl p-5">
                ...
                <div className="grid grid-cols-3 gap-2 mb-3">
                  <button onClick={async ()=>{
                    const updated = {...adminResult, hasTestAccess: true, isPremium: false};
                    await ls.set('u_' + adminResult.username, JSON.stringify(updated));
                  ...
                  <button onClick={async ()=>{
                    const updated = {...adminResult, hasTestAccess: true, isPremium: true};
                    await ls.set('u_' + adminResult.username, JSON.stringify(updated));
                    setAdminResult(updated);
                  ...
                  <button onClick={async ()=>{
                    const updated = {...adminResult, hasTestAccess: false, isPremium: false};
                    await ls.set('u_' + adminResult.username, JSON.stringify(updated));
                    setAdminResult(updated);
                ...
                <p className="text-xs text-gray-400">Changes take effect immediately. The user must log out and back in to see their updated plan.</p>
              </div>
            )}
          </div>
        </div>
