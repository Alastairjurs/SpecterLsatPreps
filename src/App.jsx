...
export default function SpecterLSATApp() {
  ...
  if (showHome && !user) return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100">
      <nav className="bg-white shadow-md"><div className="max-w-7xl mx-auto px-6 flex justify-between items-center h-20"><div className="flex items-center gap-3"><Suit /><span className="text-xl font-bold text-gray-800">Specter LSAT Prep</span></div><div className="flex items-center gap-3"><button onClick={() => setShowPricing(true)} className="text-indigo-600 font-semibold hover:text-indigo-800 px-4 py-2">Pricing</button><button onClick={() => setShowContact(true)} className="text-indigo-600 font-semibold hover:text-indigo-800 px-4 py-2">Contact Us</button><button onClick={() => setShowHome(false)} className="bg-indigo-600 text-white px-8 py-3 rounded-lg font-semibold hover:bg-indigo-700">Get Started</button></div></div></nav>
      ...
      {showPricing && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-2xl max-w-3xl w-full p-10 shadow-2xl my-8">
            <div className="flex justify-between items-center mb-8">
              <div>
                <h2 className="text-3xl font-bold text-gray-900">Simple, Transparent Pricing</h2>
                <p className="text-gray-500 mt-1">Choose the plan that fits your preparation goals</p>
              </div>
              <button onClick={() => setShowPricing(false)} className="text-gray-400 hover:text-gray-600 text-3xl font-bold leading-none">x</button>
            </div>
            <div className="grid md:grid-cols-2 gap-8 mb-8">
              <div className="border-2 border-gray-200 rounded-2xl p-8 hover:border-indigo-300 transition-all">
                ...
                <div className="mb-6"><span className="text-6xl font-bold text-gray-900">$35</span><span className="text-xl text-gray-500">/month</span></div>
                <p className="text-gray-600 mb-6">Everything you need to practice at full volume with real exam conditions.</p>
                <ul className="space-y-4 mb-8">
                  {[["2,600 total practice questions","1,300 per section across LR and RC"],["100 full practice tests","50 per section, 26 questions each"],["Randomized answer positions","No predictable patterns"],["Timed Full Test mode","Simulate real LSAT with auto-advancing sections"],["Instant score feedback","See your results after every question"],["Free Learn section","17 in-depth strategy articles included"]].map(([title, desc], i) => (
                    <li key={i} className="flex gap-3">
                      <CheckCircle className="w-5 h-5 text-green-500 flex-shrink-0 mt-0.5" />
                      <div><p className="font-semibold text-gray-800">{title}</p><p className="text-sm text-gray-500">{desc}</p></div>
                    </li>
                  ))}
                  <li className="flex gap-3"><XCircle className="w-5 h-5 text-gray-300 flex-shrink-0 mt-0.5" /><div><p className="font-semibold text-gray-400">No answer explanations</p><p className="text-sm text-gray-400">Upgrade to Premium to unlock</p></div></li>
                </ul>
                <button onClick={() => { setShowPricing(false); setShowHome(false); }} className="w-full bg-gray-900 text-white py-4 rounded-xl font-bold text-lg hover:bg-gray-800 transition-all">Get Test Access</button>
              </div>
              <div className="border-2 border-indigo-500 rounded-2xl p-8 bg-gradient-to-br from-indigo-50 to-purple-50 relative">
                ...
                <div className="mb-6"><span className="text-6xl font-bold text-gray-900">$45</span><span className="text-xl text-gray-500">/month</span></div>
                <p className="text-gray-600 mb-6">Everything in Test Access, plus the explanations that accelerate your learning.</p>
                <ul className="space-y-4 mb-8">
                  {[["Everything in Test Access","All 2,600 questions and 100 practice tests"],["Detailed correct answer explanations","Understand exactly why each answer is right"],["Detailed wrong answer explanations","Learn from every mistake you make"],["Unlocked after test completion","Complete a test to access its explanations"],["Faster score improvement","Students with explanations improve 2x faster"]].map(([title, desc], i) => (
                    <li key={i} className="flex gap-3">
                ...
                <button onClick={() => { setShowPricing(false); setShowHome(false); }} className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 text-white py-4 rounded-xl font-bold text-lg hover:from-indigo-700 hover:to-purple-700 transition-all">Get Premium Access</button>
              </div>
            </div>
